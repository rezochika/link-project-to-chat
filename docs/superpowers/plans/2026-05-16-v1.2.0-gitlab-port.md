...
Zero telegram imports — transport-portable for Telegram, future Web rooms,
and the designed-but-not-yet-built Slack and Google Chat transports.
EOF
)"
```
...
Works for any room-kind chat across transports. The current local Web UI is
direct browser-to-bot chat and emits DM-kind messages, so room context does
not apply there until a multi-user Web room model exists.
...
After implementation, a read-only audit found two release-readiness gaps:

- **Web ROOM context scope clarified.** The v1.2.0 implementation is
  transport-portable for any transport that emits `ChatKind.ROOM`, but the
  production Web UI currently emits direct browser-to-bot chats as
  `ChatKind.DM`. The Web-room acceptance claim is therefore deferred until
  WebTransport grows a real multi-user room surface. A Web transport
  characterization test pins the current DM-kind behavior.
- **`ChatHistory.record()` idempotency strengthened.** The implementation now
  matches the documented `(chat, msg_id)` no-duplicate contract instead of
  only skipping duplicate tail entries.
- **Version metadata bumped.** The committed package metadata and
  `link_project_to_chat.__version__` now report `1.2.0`.
